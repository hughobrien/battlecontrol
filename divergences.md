...
| D19 | Residual ore, unit animation, and palette-cycle differences remain after best-offset frame alignment. | Open | Fresh fixed-seed 60 FPS sequence comparison at `/tmp/battlecontrol/d7-sequence-align-20260522-155843` aligns best at native `wine+2` frames, but still leaves tactical differences that look like ore tile/overlay variation, unit animation phase, palette cycling, or per-object animation counters rather than static renderer geometry. Do not bury these under D7: D7 is the capture/present boundary; D19 is the determinism problem. Next probes should compare Wine/native object/overlay state at the aligned pair, identify whether ore frame selection uses PRNG or a cell-derived animation phase, and check unit/infantry animation counters and `Color_Cycle()` phase under the sequence harness. |
