#!/usr/bin/env bash
# TIM-63 measurement: pass 32.
#
# Group A type/macro continuation of TIM-59 (pass-31). Pass-31 cleared
# 7/7 function-shim singletons (FindFirstFile, MessageBox, socket,
# NSPROTO_IPX, WSAAsyncSelect, timeSetEvent, TEXT_NO_RAM follow-ups)
# and surfaced a fresh batch of type/macro singletons in the same
# already-touched headers. Pass-32 lands those macro definitions so the
# DDRAW/WSPUDP TUs flip OK and the WSPIPX/WSPROTO TUs are pre-positioned
# for the pass-33 setsockopt/getsockopt function-shim bundle.
#
# Pre baseline:
#   pass 31 (TIM-59) post-commit 9eaa9dd : 253 OK / 48 Fail / 301 Total.
#   (Includes TIM-61 source-level min/max ADL fix that landed after TIM-59.)
#
...
#   pass 32 (TIM-63, this run)            : 254  -- ΔOK 0 vs rebaselined; 4
...
#                                                                (queue pass-33).
#
# Measurement only -- source fixes live in TIM-63.
...
LOG_FILE="$LOG_DIR/first-compile-pass32.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass32.summary.txt"
ATTRIB_FILE="$LOG_DIR/first-compile-pass32.attribution.txt"
...
{
    echo "# TIM-63 first compile attempt -- pass 32 (post TIM-63 bundle-mode type/macro sweep: E_FAIL/E_INVALIDARG/E_OUTOFMEMORY/E_NOTIMPL/INADDR_ANY/SOL_SOCKET/SO_BROADCAST/SO_LINGER/SO_SNDBUF/SO_RCVBUF/SO_ERROR/IPX_PTYPE/IPX_FILTERPTYPE)"
    echo "# host: $(uname -srm)"
