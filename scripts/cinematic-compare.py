...
def parse_vqhd(vqa_path: str) -> dict | None:
    ...
    while pos + 8 <= len(data):
        ...
        if tag == b'VQHD':
            ...
            # 16:maxBlocks(2)
            version, flags, numFrames, width, height = struct.unpack_from('<HHHHH', body, 0)
            blockW, blockH, fps, cbParts = struct.unpack_from('<BBBB', body, 10)
            numColors, maxBlocks = struct.unpack_from('<HH', body, 14)
            if blockW == 0: blockW = 4
            if blockH == 0: blockH = 2
            if maxBlocks == 0: maxBlocks = 512
            return {
                'numFrames': numFrames, 'width': width, 'height': height,
                'fps': fps, 'blockW': blockW, 'blockH': blockH,
                'cbParts': cbParts or 1, 'maxBlocks': maxBlocks,
            }
...
def _save_diff_png(path: str, pixels_a: bytearray, pixels_b: bytearray, w: int, h: int):
    ...
    for y in range(h):
        ...
        for x in range(w):
            i = (y * w + x) * 3
            for c in range(3):
                raw.append(min(255, abs(int(pixels_a[i+c]) - int(pixels_b[i+c])) * 4))
...
def _lcw_decode(src: bytes, dst_cap: int) -> bytearray:
    ...
    sp = 0; dst = bytearray(dst_cap); dp = 0
    sv = memoryview(src)
    while sp < len(sv) and dp < dst_cap:
        ...
        if op == 0x80:
            ...
            if sp >= len(sv): break
            count = (op >> 4) + 3; offset = ((op & 0x0f) << 8) | sv[sp]; sp += 1
            cp = dp - offset
            if cp < 0: cp = 0
            for _ in range(count):
                if dp >= dst_cap: break
                dst[dp] = dst[cp]; dp += 1; cp += 1
        elif not (op & 0x40):
            count = op & 0x3f
            for _ in range(count):
                if sp >= len(sv) or dp >= dst_cap: break
                dst[dp] = sv[sp]; dp += 1; sp += 1
        elif op == 0xfe:
            if sp + 2 >= len(sv): break
            count = sv[sp] | (sv[sp+1] << 8); sp += 2
            fill = sv[sp]; sp += 1
            for _ in range(count):
                if dp >= dst_cap: break
                dst[dp] = fill; dp += 1
        elif op == 0xff:
            if sp + 3 >= len(sv): break
            count = sv[sp] | (sv[sp+1] << 8); sp += 2
            off   = sv[sp] | (sv[sp+1] << 8); sp += 2
            cp = off
            for _ in range(count):
                if dp >= dst_cap: break
                dst[dp] = dst[cp] if cp < dp else 0; dp += 1; cp += 1
        else:
            if sp + 1 >= len(sv): break
            count = (op & 0x3f) + 3
            off   = sv[sp] | (sv[sp+1] << 8); sp += 2
            cp = off
            for _ in range(count):
...
def decode_vqa_frame_inline(vqa_data: bytes, target_frame: int) -> bytearray | None:
    ...
    le8  = lambda d, o: struct.unpack_from('<B', d, o)[0]
    ...
    while pos + 8 <= len(vqa_data) and frame_num < numFrames:
        ...
        for stag, ssz, sbody in chunks:
            if stag == b'CBP0':
                ...
                partial_countdown -= 1
                if partial_countdown <= 0:
                    decomp = _lcw_decode(next_codebook_buffer[:next_cb_idx], codebook_size)
                    codebook[:len(decomp)] = decomp
                    next_cb_idx = 0
                    partial_countdown = cbParts
        ...
        if frame_num == target_frame:
            ...
            for i in range(vqaW * vqaH):
                pidx = framebuf[i] * 3
                r = palette[pidx];   r = ((r << 2) & 0xFF) | ((r >> 4) & 0x03)
                g = palette[pidx+1]; g = ((g << 2) & 0xFF) | ((g >> 4) & 0x03)
                b = palette[pidx+2]; b = ((b << 2) & 0xFF) | ((b >> 4) & 0x03)
...
def compare_vqa(vqa_path: str, label: str, num_frames: int, out_base: str,
                threshold: int, quiet: bool) -> dict:
    ...
    if not quiet:
        ...
        print(f'  [{result["status"]}] {label}: frame={compare_frame}/{num_frames} '
...
def main() -> int:
    ...
    ap.add_argument('mix_path', nargs='?', default=MAIN_MIX_DEFAULT,
    ...
    ap.add_argument('--vqa-dir', default=None,
    ...
    print(f'=== TIM-705 Cinematic Comparison ===')
    ...
    for vp, lbl, nf in chosen:
        vqa_out = os.path.join(out_dir, lbl.replace('~', '_approx').replace('.VQA', '').replace('.vqa', ''))
    ...
    for r in passed:
        ...
        print(f'    [PASS] {r["label"]}: p99={r.get("p99","?")} mean={r.get("mean","?")}{ssim_str}')
    if failed:
        print(f'  FAIL: {len(failed)}/{len(results)}')
        for r in failed:
            print(f'    [FAIL] {r["label"]}: p99={r.get("p99","?")} diff={r.get("diff_image","?")}')
    if skipped:
    ...
    with open(report_path, 'w') as fh:
        json.dump({'results': results, 'summary': {
            ...
            'total': len(results), 'threshold': args.threshold,
        }}, fh, indent=2)
    print(f'\nReport: {report_path}')
