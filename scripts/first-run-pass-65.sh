#!/usr/bin/env bash
# TIM-229 pass-65: fix invisible menu button text — DIALOG_BLUE Color override
#
# Root cause: ColorRemaps[PCOLOR_DIALOG_BLUE].Color from PALETTE.CPS col 4
#   row 10 = palette index 194, which is visually indistinguishable from the
#   button background (Background field) under the Linux/SDL palette mapping.
#   TPF_MEDIUM_COLOR fills fontpalette[4..15] with fore->Color; if Color==Background
#   the text is invisible.
#
# Fix: INIT.CPP Init_Color_Remaps — set Color=WHITE (15) for PCOLOR_DIALOG_BLUE,
#   mirroring the existing BrightColor=WHITE hardcode.
#
# Delta from pass-64: INIT.CPP only — Color override + Background in debug log.
#
# Run from repo root:
#   bash scripts/first-run-pass-65.sh
...
PASS_DIR="$REPO_ROOT/build/first-run-pass-65"
...
echo "--- DIALOG_BLUE init values (pass-65 fix) ---"
grep -a "\[RA\] DIALOG_BLUE" "$LOG" | head -5 || echo "(not fired)"
echo ""

echo "--- Button text print (STP) ---"
grep -a "\[STP\]" "$LOG" | grep -v "^$" | head -20 || echo "(none)"
echo ""
