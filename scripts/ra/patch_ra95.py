    ...
    return {
        ...
        "focus-wait-skip": PatchSpec(
            ...
            edits=(
                ByteEdit(
                    0x154005,
                    bytes.fromhex("0f8455ffffff"),
                    b"\x90" * 6,
                    "focus wait branch 1",
                ),
                ByteEdit(
                    0x15F2F1,
                    bytes.fromhex("0f847bffffff"),
                    b"\x90" * 6,
                    "focus wait branch 2",
                ),
                ByteEdit(
                    0x15F583,
                    bytes.fromhex("0f847affffff"),
                    b"\x90" * 6,
                    "focus wait branch 3",
                ),
            ),
        ...
        "game-in-focus": PatchSpec(
            id="game-in-focus",
            purpose=(
                "Quarantined: confirmed bad Session.Type write previously "
                "mislabeled as GameInFocus."
            ),
            status="quarantined",
            default_allowed=False,
    ...
    *,
    ...
    for patch_id in selected:
        ...
        if patch_id == "scenario":
            ...
            side_replacement = (
                bytes.fromhex("eb07")
                if effective_side == "soviet"
                else bytes.fromhex("9090")
            )
            ...
            if actual == side_replacement:
                ...
                raise PatchError(
                    ...
                    f"expected {side_expected.hex()} or "
                    f"{side_replacement.hex()}, got {actual.hex()}"
                )
            edits.append(
                {
            ...
            raise PatchError(
                "game-in-focus: quarantined patch is non-applicable in unified patcher"
            )
        else:
            edits = _apply_static_patch(data, spec)
