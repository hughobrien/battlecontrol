...
SPIN_ORIG_AFTER_FOCUS_SKIP = bytes.fromhex('803db8b66600009090909090 90'.replace(' ',''))
...
def patch(path: str, dry_run: bool = False) -> int:
    ...
    entry_already = bytes(data[ENTRY_FILE_OFFSET:ENTRY_FILE_OFFSET + 15]) == ENTRY_PATCHED_BYTES
    cave_already = bytes(data[CAVE_FILE_OFFSET:CAVE_FILE_OFFSET + 22]) == CAVE_PATCHED_BYTES
    spins_already = all(
        bytes(data[off:off + 13]) == SPIN_PATCHED_BYTES for off in SPIN_LOOP_CMP_OFFSETS
    )
    ...
    if digest not in ACCEPTED_INPUT_SHA256:
        ...
        print(f"       accepted inputs (any of):")
    ...
    for off in SPIN_LOOP_CMP_OFFSETS:
        ...
        if actual == SPIN_ORIG_PRE_FOCUS_SKIP[off]:
            print(f"warn: focus-skip-patch.py not yet applied at 0x{off:x} — proceeding")
            continue
        print(f"ERROR: unexpected bytes at spin-loop site 0x{off:x}: {actual.hex()}")
        return 1

    if dry_run:
        print(f"  DRY RUN: entry detour, code cave, {len(SPIN_LOOP_CMP_OFFSETS)} spin-loop rewrites")
        return 0
    ...
    print(f"  Patched code cave at 0x{CAVE_FILE_OFFSET:x} ({len(CAVE_PATCHED_BYTES)} bytes)")
    print(f"  Rewrote {len(SPIN_LOOP_CMP_OFFSETS)} spin-loop cmp+nops -> mov [GIF],1 + nops")
