...
class NativeCapture:
    ...
    def capture_mission(
        ...
        try:
            ...
            mouse_x, mouse_y = capture_game_mouse_position(640, 400)
            env["RA_CAPTURE_MOUSE_X"] = str(mouse_x)
            env["RA_CAPTURE_MOUSE_Y"] = str(mouse_y)
            if os.environ.get("RA_CAPTURE_STATE_DUMP", "0") not in ("", "0"):
                env["RA_CAPTURE_STATE_FILE"] = str(output_dir / "native-state.txt")
    ...
    def capture_mission_sequence(
        ...
        try:
            ...
            env["RA_CAPTURE_MOUSE_Y"] = str(mouse_y)
            if os.environ.get("RA_CAPTURE_STATE_DUMP", "0") not in ("", "0"):
                env["RA_CAPTURE_STATE_FILE"] = str(
                    output_dir / "native-sequence-state.txt"
                )
                env["RA_CAPTURE_STATE_APPEND"] = "1"
            ra_proc = subprocess.Popen(
                [str(self.ra_bin)],
                env=env,
                cwd=str(staged_data_dir),
