...
def center_mouse(disp: str, width: int, height: int):
    ...
    x, y = capture_mouse_position(width, height)
    subprocess.run(
...
def capture_mouse_position(width: int, height: int) -> tuple[int, int]:
    """Return the deterministic capture mouse position for a game window."""
    x = int(os.environ.get("BC_CAPTURE_MOUSE_X", str(width // 2)), 0)
    y = int(os.environ.get("BC_CAPTURE_MOUSE_Y", "60"), 0)
    x = min(max(x, 0), width - 1)
    y = min(max(y, 0), height - 1)
    return x, y


def capture_game_mouse_position(width: int, height: int) -> tuple[int, int]:
    """Return the deterministic in-game mouse coordinate for owned engines."""
    x = int(os.environ.get("BC_CAPTURE_GAME_MOUSE_X", str(width // 2)), 0)
    y = int(os.environ.get("BC_CAPTURE_GAME_MOUSE_Y", str(height + 100)), 0)
    return x, y
