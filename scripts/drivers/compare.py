...
def compare_pair(golden_path: str, capture_path: str, label: str,
                 diff_dir: str, threshold_ssim=0.90) -> dict:
    ...
    r = subprocess.run(
        ["python3", str(parity_script), golden_path, capture_path,
         "--label", label,
         "--threshold-ssim", str(threshold_ssim),
         "--diff-out", str(diff_out),
         "--json"],
        capture_output=True, text=True, timeout=60)
    passed = r.returncode == 0
    data = {"pair": label, "passed": passed, "ssim": 0.0,
            "p99": 255, "diff_path": str(diff_out), "stdout": r.stdout}
    try:
...
def full_report(captures: dict, output_dir: str,
                threshold_ssim=0.90) -> dict:
    ...
    for i in range(len(targets)):
        for j in range(i + 1, len(targets)):
            ...
            r = compare_pair(str(captures[a]), str(captures[b]),
                             f"{a}-vs-{b}", str(diff_dir), threshold_ssim)
            results.append(r)
