#!/usr/bin/env bash
# TIM-59 measurement: pass 31.
#
# Group A function-shim continuation of TIM-56 (pass-30). Pass-30 cleared
# 7/7 type/macro singletons (WIN32_FIND_DATA, MB_*, SW_*, sockaddr_in,
# SOCK_*/AF_*, FD_*, TIME_*) but each candidate TU advanced to a
# function-not-declared first error on the same TU. Pass-31 lands the
# matching function declarations / inline shims to flip those TUs OK.
#
# Pre baseline:
#   pass 30 (TIM-56) post-commit e720168 : 249 OK / 52 Fail / 301 Total.
#
# Difference relative to pass 30 is upstream-only (Group A function-shim
# bundle in three already-touched headers; no shim restructuring, no
# flag changes, no engine source edits):
#   - TIM-59 fix 1: linux/win32-stubs/windows.h gains FindFirstFileA /
#                   FindNextFileA / FindClose static-inline shims +
#                   FindFirstFile / FindNextFile macro aliases. The
#                   stub returns INVALID_HANDLE_VALUE so engine while-
#                   loops in SESSION.CPP:1325/1446 terminate cleanly.
#   - TIM-59 fix 2: linux/win32-stubs/windows.h gains MessageBoxA
#                   static-inline shim (returns IDOK) + MessageBox
#                   macro alias + IDOK/IDCANCEL/IDYES/IDNO return-code
#                   set. Targets WIN32LIB/DDRAW.CPP:90+ chain.
#   - TIM-59 fix 3: linux/win32-stubs/winsock.h gains LPSOCKADDR
#                   typedef family (struct sockaddr forward-declared
#                   opaque + SOCKADDR/PSOCKADDR/LPSOCKADDR aliases) so
#                   the (LPSOCKADDR)&addr casts in WSPUDP.CPP /
#                   WSPIPX.CPP parse.
#   - TIM-59 fix 4: linux/win32-stubs/winsock.h gains NSPROTO_IPX
#                   protocol-selector macro (1000). Targets WSPIPX.CPP
#                   :98 socket() call.
#   - TIM-59 fix 5: linux/win32-stubs/winsock.h gains socket / bind /
#                   sendto / recvfrom static-inline shims. Stubs return
#                   INVALID_SOCKET / SOCKET_ERROR so engine bail-out
#                   logic at the call sites takes the failure branch.
#   - TIM-59 fix 6: linux/win32-stubs/winsock.h gains WSAAsyncSelect /
#                   WSACancelAsyncRequest static-inline shims (return
#                   SOCKET_ERROR). Targets WSPROTO.CPP:187/190/215.
#   - TIM-59 fix 7: linux/win32-stubs/mmsystem.h gains LPTIMECALLBACK
#                   typedef + timeSetEvent / timeKillEvent static-inline
#                   shims (return 0 / TIMERR_NOERROR). Targets WIN32LIB
#                   /TIMERINI.CPP:122/156. The Timer_Callback signature
#                   (UINT, UINT, DWORD x3) matches the typedef exactly.
#
# Same harness as passes 7-30 -- same flags, same shim regen, same
# stubs. Difference is only the function-shim additions above.
#
# Reference: TIM-56 measurement (pass 30).
#
...
LOG_FILE="$LOG_DIR/first-compile-pass31.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass31.summary.txt"
ATTRIB_FILE="$LOG_DIR/first-compile-pass31.attribution.txt"
...
{
    echo "# TIM-59 first compile attempt -- pass 31 (post TIM-59 bundle-mode function-shim sweep: FindFirstFile/MessageBox/socket/bind/sendto/recvfrom/WSAAsyncSelect/WSACancelAsyncRequest/NSPROTO_IPX/timeSetEvent/timeKillEvent/LPSOCKADDR/LPTIMECALLBACK)"
    echo "# host: $(uname -srm)"
