...
def main() -> int:
    ...
    try:
        ...
        for vqa_path in vqa_files:
            ...
            for fidx in frames:
                for pattern, name in [
                    (f"raw_{fidx + 1:04d}.png", "committed"),
            ...
            for fidx in sorted(golden_frames):
                ...
                if not os.path.exists(tpath):
                    print(
                        f"  frame {fidx + 1:4d}: SKIP  (decoder did not produce output)"
                    )
                    continue
                ...
                if "error" in diff:
                    ...
                    all_pass = False
                    results.append(
                        {"vqa": vqa_stem, "frame": fidx + 1, "status": "ERROR"}
                    )
                    continue
        ...
        print(
            f"Results: {n_pass} PASS  {n_fail} FAIL  (threshold p99 ≤ {args.threshold})"
        )
