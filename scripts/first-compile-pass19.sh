#!/usr/bin/env bash
# TIM-43 measurement: pass 19.
#
# Re-baseline after TIM-42 (commit a996e97 -- qualify max/min and add
# fwd decls in REDALERT/LIST.H) AND TIM-43 (this ticket -- add a
# `Fancy_Text_Print(..., ColorType, ...)` overload in FUNCTION.H that
# delegates to the canonical signature with nullptr, and forward-decl
# that overload in LIST.H so phase-1 lookup resolves it).
#
# Pre baseline (pass 18, TIM-41-confirmed): 95 OK / 206 Fail / 301 Total.
# Top of pass-18 histogram: list.h:267 (max) -> after TIM-42, relocated
# to list.h:301 (Fancy_Text_Print) at ~181 TUs (predicted, no measure).
# This pass is the first measurement that includes BOTH TIM-42's LIST.H
# fwd-decl block AND TIM-43's Fancy_Text_Print overload.
#
# Same harness as passes 7-18 -- same flags, same shim regen, same
# stub set. Differences relative to pass 18 are upstream:
#   - TIM-42: REDALERT/LIST.H std::max/std::min qualification + fwd
#     declarations of Conditional_{Hide,Show}_Mouse, Draw_Box, SeenBuff
#   - TIM-43: REDALERT/FUNCTION.H Fancy_Text_Print(ColorType) overload,
#             REDALERT/LIST.H fwd decl of that overload
#
...
#   pass 16 (TIM-38)                      : 95
#   pass 18 (TIM-40)                      : 95
#   pass 19 (TIM-42 + TIM-43, this run)   : ?
#
# Measurement only -- source fix lives in TIM-43.
...
LOG_FILE="$LOG_DIR/first-compile-pass19.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass19.summary.txt"
ATTRIB_FILE="$LOG_DIR/first-compile-pass19.attribution.txt"
...
{
    echo "# TIM-43 first compile attempt -- pass 19 (post TIM-42 + TIM-43)"
    echo "# host: $(uname -srm)"
