#!/usr/bin/env bash
# TIM-50 measurement: pass 25.
#
# Re-baseline after TIM-50 (this ticket -- chain-audit scope-change.
# Captured the post-shim include graph and full-diagnostic stream for
# AIRCRAFT.CPP under the pass-24 flags, found exactly ONE prefix
# diagnostic shared by the 179-TU cohort: `int cdecl Dialog_Message(...)`
# at FUNCTION.H:601. Bare lowercase `cdecl` mirrors the TIM-29 fix shape
# for bare far/near/pascal -- single-line empty-#define in
# msvc-compat.h. Spot-check on CONFDLG.CPP / 2KEYFRAM.CPP / AADATA.CPP /
# ABSTRACT.CPP confirmed identical prefix; post-prefix diagnostics
# already differ per TU. Expectation: cohort fragments on this pass.
#
# Pre baseline (pass 24, TIM-49-confirmed): 95 OK / 206 Fail / 301 Total.
# Top of pass-24 histogram: function.h:601 = 179 TUs (eighth relocation
# of the same large cohort: list.h:267 -> list.h:301 -> drop.h:160 ->
# tevent.h:172 -> DSOUND.H:111 -> DSOUND.H:140 -> externs.h:123 ->
# function.h:601).
#
# Same harness as passes 7-24 -- same flags, same shim regen, same
# stub set. Difference relative to pass 24 is upstream:
#   - TIM-50: linux/win32-stubs/msvc-compat.h gains
#             `#ifndef cdecl / #define cdecl / #endif` adjacent to the
#             TIM-29 far/near/pascal block. Empty-define is non-
#             disruptive: every site of bare `cdecl` in REDALERT/ is a
#             calling-convention qualifier on a prototype/definition,
#             never an identifier. MEMCHECK.H:376 already #defines
#             cdecl in its own non-Borland branch, confirming the
#             upstream-intended treatment.
#
...
#   pass 23 (TIM-47)                      : 95
#   pass 24 (TIM-49)                      : 95
#   pass 25 (TIM-50, this run)            : ?
#
# Measurement only -- source fix lives in TIM-50.
...
LOG_FILE="$LOG_DIR/first-compile-pass25.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass25.summary.txt"
ATTRIB_FILE="$LOG_DIR/first-compile-pass25.attribution.txt"
...
{
    echo "# TIM-50 first compile attempt -- pass 25 (post TIM-50 bare-cdecl shim in msvc-compat.h)"
    echo "# host: $(uname -srm)"
