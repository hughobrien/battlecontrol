...
def lcw_decode(src_bytes: bytes, dst_cap: int) -> bytearray:
    ...
    while sp < len(src) and dp < dst_cap:
        ...
        if op == 0x80:
            ...
            count = op & 0x3f
            for _ in range(count):
                if sp >= len(src) or dp >= dst_cap: break
            ...
            for _ in range(count):
                if dp >= dst_cap: break
                dst[dp] = dst[cp] if cp < dst_cap else 0; dp += 1; cp += 1
        else:
...
# ---------------------------------------------------------------------------
def blit_vqa_frame(pixels_src: bytearray, vqaW: int, vqaH: int,
                   dst: bytearray, dstPitch: int, scrW: int, scrH: int):
...
# Main VQA decoder
# ---------------------------------------------------------------------------
def decode_vqa(vqa_path: str, outdir: str, frames_to_dump=None,
               screen_w: int = 640, screen_h: int = 480):
    """
    ...
    while pos + 8 <= len(data):
        ...
        if tag == b'VQHD':
            ...
            hdr = dict(numFrames=numFrames, width=width, height=height,
                       blockW=blockW, blockH=blockH, fps=fps, cbParts=cbParts,
                       numColors=numColors, maxBlocks=maxBlocks,
                       flags=flags, freq=freq, channels=channels, bits=bits)
            print(f"VQHD: {width}x{height} fps={fps} frames={numFrames}")
            print(f"  blockW={blockW} blockH={blockH} cbParts={cbParts} maxBlocks={maxBlocks}")
            pos = npos
            break
...
if __name__ == '__main__':
    ...
    ap.add_argument('--frames', default='0,1,29,59', help='0-indexed frame numbers to dump (comma-separated)')
    ...
    decode_vqa(args.vqa, args.outdir, frames_to_dump=frames,
               screen_w=args.screenw, screen_h=args.screenh)
