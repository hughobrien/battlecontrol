...
# — helpers —
gate_pass() { PASSED_GATES+=("$1"); echo "  PASS: $1"; }
gate_skip() { SKIPPED_GATES+=("$1 ($2)"); echo "  SKIP: $1 — $2"; }
gate_fail() { FAILED_GATES+=("$1"); echo "  FAIL: $1"; }

have() { command -v "$1" &>/dev/null; }

# — check what's available —
check_native()   { have g++ && have cmake && have ninja && have pkg-config && pkg-config --exists sdl2 2>/dev/null; }
check_wasm()     { have emcmake; }
check_wasm_run() { have emcmake && have node && have npx; }
...
if [[ "$MODE" != "--wasm-only" ]] && check_native; then
    ...
    gate_skip "G1: native build" "missing toolchain (g++/cmake/ninja/SDL2)"
fi
