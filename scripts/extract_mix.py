...
def parse_mix(data: bytes):
    ...
    if first == 0:
        ...
        is_digest    = bool(flags & 0x01)
        is_encrypted = bool(flags & 0x02)
        if is_encrypted:
            print("[MIX] File has RSA-encrypted index — cannot extract without key.",
                  file=sys.stderr)
            return None, 0
...
COMMON_RA_NAMES = [
    "ENGLISH.VQA", "TITLE.PCX", "HIRES.MIX", "LORES.MIX", "LORES1.MIX", "HIRES1.MIX",
    "LOCAL.MIX", "REDALERT.MIX", "MAIN.MIX", "EXPAND.MIX", "EXPAND2.MIX",
    "SPEECH.MIX", "SCORES.MIX", "ALLIES1.VQA", "ALLIES2.VQA", "SOVIET1.VQA",
    "INTRO.VQA", "INGAME.MIX", "TRANSIT.MIX", "GENERAL.MIX",
    "ALLY1.VQA", "ALLY2.VQA", "SOV1.VQA", "SOV2.VQA",
    "ALLYMTN.VQA", "MTNRULES.VQA", "BKGROUND.PCX", "CONQUER.MIX",
...
def main():
    ...
    ap.add_argument('--extract-all-known', action='store_true',
