...
def native_decode_frame(vqa_data, target_frame):
    ...
    from vqa_decode_verify import lcw_decode, be32
    ...
    for ci in range(256):
        codebook[(0xFF00 + ci) * cbEntrySize : (0xFF00 + ci + 1) * cbEntrySize] = (
            bytes([ci]) * cbEntrySize
        )
        codebook[(0x0F00 + ci) * cbEntrySize : (0x0F00 + ci + 1) * cbEntrySize] = (
            bytes([ci]) * cbEntrySize
        )
...
def main():
    ...
    for i, frame_idx in enumerate(indices):
        ...
        print(
            f"  frame {frame_idx}: p99={diff['p99']} p95={diff['p95']} mean={diff['mean']} max={diff['max']} [{status}]"
        )
