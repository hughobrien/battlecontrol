#!/usr/bin/env bash
# TIM-23 measurement: pass 9.
#
# Re-baseline after TIM-22 (Bucket 4 tier-2 -- header self-containment
# for HOUSE.H + GSCREEN.H + TEMPLATE.H + the JSHELL.H residual). TIM-21
# pinned the cause of the 88-OK plateau on the Big-Six post-fix
# landscape: every failing TU's first 20 errors fired in HOUSE.H,
# GSCREEN.H, TEMPLATE.H, or the JSHELL.H residual, all of which sat
# below -fmax-errors=20 and starved the platform-seam buckets of
# honest counts. With those four headers now self-contained, the
# cascade should finally clear and OK count should jump off 88 toward
# the TIM-13 projection (~190+).
#
# Same harness as passes 7 and 8 -- same flags, same shim regen, same
# stub include path. The only difference relative to pass 8 is
# upstream: tier-2 headers now compile in isolation.
#
...
LOG_FILE="$LOG_DIR/first-compile-pass9.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass9.summary.txt"
...
CXXFLAGS=(
    ...
    # tier-2 self-containment) are still needed.
...
{
    echo "# TIM-23 first compile attempt -- pass 9 (post TIM-22)"
    echo "# host: $(uname -srm)"
...
# Tally include-not-found errors so we can compare against pass 8's
