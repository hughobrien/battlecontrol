#!/usr/bin/env bash
# TIM-21 measurement: pass 8.
#
# Re-baseline after TIM-20 (header self-containment for the Big Six +
# monoc.h redefinition). The Big Six -- the six most heavily-included
# REDALERT headers -- now provide their own forward declarations and
# typename annotations, demolishing the bucket-4 cascade where a TU
# would parse fine but its dependents would not because a sibling
# header was implicitly relying on its includer's order.
#
# Same harness as pass 7 -- same flags, same shim regen, same stub
# include path. The only difference relative to pass 7 is upstream:
# the Big Six headers now compile in isolation (no more "use of
# undeclared identifier" cascades that depended on includer order),
# and monoc.h's redefinition collision is gone.
#
...
LOG_FILE="$LOG_DIR/first-compile-pass8.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass8.summary.txt"
...
{
    echo "# TIM-21 first compile attempt -- pass 8 (post TIM-20)"
    echo "# host: $(uname -srm)"
...
# Tally include-not-found errors so we can compare against pass 7's
