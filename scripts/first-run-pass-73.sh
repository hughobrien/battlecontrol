#!/usr/bin/env bash
# TIM-273 pass-73: fix black sidebar panel background
#
# Root-cause hypothesis: CONQUER.MIX is registered but not cached, so
# MFCD::Retrieve("SIDE1NA.SHP") returns NULL.  SidebarShape/Middle/Bottom
# are all NULL → CC_Draw_Shape skips drawing → panel stays black.
#
# Changes from pass-72:
#   - REDALERT/INIT.CPP: add MFCD::Cache("CONQUER.MIX") in Init_Secondary_Mixfiles
#     so Retrieve() can return pointers for shapes stored in CONQUER.MIX
#   - REDALERT/SIDEBAR.CPP: add diagnostic fprintf in One_Time and Init_IO
#     to log sidebar shape pointer state
#
# Pass criterion:
#   - SIDEBAR log shows top/mid/bot all non-NULL after Init_IO
#   - frame500 shows textured/coloured sidebar panel background (not solid black)
#   - nz_pixels > 76531 (pass-72 baseline)
#
# Run from repo root:
#   bash scripts/first-run-pass-73.sh
...
PASS_DIR="$REPO_ROOT/build/first-run-pass-73"
...
echo "--- Sidebar shape pointer state ---"
grep -a "\[SIDEBAR\]" "$LOG" | head -10 || echo "(no sidebar log)"
echo ""
...
print(nz)
" 2>/dev/null)
        echo "frame500 nz_pixels=$NZ (pass-72 baseline=76531)"
    fi
