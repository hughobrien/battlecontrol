...
[[ -f "$CNC_DDRAW_DIR/ddraw.dll" ]] || { echo "FAIL: cnc-ddraw missing at $CNC_DDRAW_DIR"; exit 1; }
...
if ! kill -0 $RA_PID 2>/dev/null; then
    echo "FAIL: RA died — see $ARTIFACT_DIR/wine.log"; exit 3
fi
...
if [[ ! -f "$TARGET" ]]; then
    echo "  FAIL frame-500.png missing"; PASS=0
else
    sz=$(stat -c%s "$TARGET")
