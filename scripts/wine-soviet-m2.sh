#!/usr/bin/env bash
# TIM-857 — Drive RA95.EXE into Soviet Mission 2 under headless Wine and
# capture frame-100 / frame-250 / frame-500 gameplay screenshots.
#
# How this reaches Soviet Mission 2
# ─────────────────────────────────
# The script swaps the auto-start target to Soviet M2 (SCU02EA.INI) by
# applying two patches over the standard chain:
#
#   1. soviet-cdlabel-patch.py   — zeros the first byte of "CD2" in
#       _CD_Volume_Label[] (file offset 0x1BFCBB) instead of "CD1".
#       Wine's empty CIFS volume label matches index 1 (Soviet) instead
#       of index 0 (Allied).  Get_CD_Index returns 1, CurrentCD = 1, and
#       the IsFromInstall branch at INIT.CPP:1032-1036 selects Soviet.
#
#   2. ra-scenario-patch.py SCU02EA — replaces all "SCU01EA.INI" byte
#       sequences in the binary with "SCU02EA.INI" so the faction-
#       selection code path loads M2 instead of M1.
#
# Combined effect: cdlabel picks Soviet, scenario patch loads M2.
#
# Scenario data location
# ──────────────────────
# SCU02EA.INI lives in MAIN.MIX at raw offset 0xF62A14 as an embedded
# text block.  The game reads it via MixFileClass::Retrieve — no loose
# file is needed in the staging directory.
#
# Rendering
# ─────────
...
#
# Outputs in $ARTIFACT_DIR (default: e2e/report/data/wine-ra-soviet-m2/):
#   frame-0.png    — pre-Options, Soviet M2 mission rendering
#   frame-100.png  — Options dialog over Soviet M2 terrain (paused)
#   frame-250.png  — post-resume, Soviet M2 gameplay in progress
#   frame-500.png  — Soviet M2 ~33 s after mission start
...
WINEPREFIX="${WINEPREFIX:-$HOME/.wine-tim857-soviet-m2}"
...
STAGE=$(mktemp -d "/tmp/tim857-soviet-m2-XXXX")
...
# Apply scenario override: SCU01EA -> SCU02EA (Soviet M2)
...
# ─── Capture sequence ────────────────────────────────────────────────────────
#
# Boot under the soviet-cdlabel + ra-scenario patch chain:
#   * 0-3 s   : Wine launches RA95.EXE
#   * 3-6 s   : Get_CD_Index returns 1 (CD2 label matched empty Wine label)
#                CurrentCD = 1, IsFromInstall = true, scenario set to
#                SCU02EA.INI (patched from SCU01EA) by INIT.CPP:1032-1036
#   * frame 0 : Soviet M2 rendered ~6 s after window appears.
...
echo
if [[ "$PASS" -eq 1 ]]; then
    echo "RESULT: PASS -- frame-500 shows non-black Soviet M2 ($SCENARIO) terrain"
    exit 0
else
