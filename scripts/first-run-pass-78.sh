#!/usr/bin/env bash
# TIM-280 pass-78: fix top bar rendering (TabClass::Draw_It)
#
# Root cause: entire Draw_It drawing block was wrapped in #if (0)
#   (comment: "Disable tab drawing for menu, credits buttons etc. ST - 5/27/2019")
#   Re-enabled for native Linux port — remastered DLL overlay is not in use.
#
# Changes from pass-77:
#   - REDALERT/TAB.CPP: Remove #if(0)/#endif wrapper around Draw_It drawing block
#   - REDALERT/TAB.CPP: One_Time: add [TAB] log and drop unused RawFileClass local
#
# Pass criterion:
#   - Compile ok, link ok, smoke test runs 90s without SIGSEGV
#   - frame500 top bar (x=0-639, y=0-15): non-black pixel count >= 1000
#
# Run from repo root:
#   bash scripts/first-run-pass-78.sh
...
PASS_DIR="$REPO_ROOT/build/first-run-pass-78"
...
echo "--- TAB diagnostic ---"
grep -a "TAB\|TABS" "$LOG" | head -10 || echo "(no TAB log)"
echo ""
...
print("Zone analysis:")
top = zone(0, width,   0,  16, "Top bar    y=0-15  ")
...
zone(480, 640, 400, 480, "Sidebar bt x=480+ y=400-479")

print(f"\nPass criterion: top_bar >= 1000 non-black pixels (got {top})")
if top >= 1000:
    print("PASS: top bar is rendering")
else:
    print("FAIL: top bar still mostly black")
PYEOF
    fi
