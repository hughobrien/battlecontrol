#!/usr/bin/env bash
# TIM-10 measurement: pass 4.
#
# Re-baseline after TIM-8 (Ctrl-Z scrub of 7 upstream headers) and TIM-9
# (windows.h Win32 type taxonomy + union REGS) landed. Same harness as
# pass 3 — same flags, same shim regen, same stub include path. The only
# differences relative to pass 3 are upstream: more bytes parse cleanly
# now, and more of windows.h exists.
#
# Pass progression:
#   pass 1 (no shim)              : 37 OK / 264 FAIL
#   pass 2 (lowercase symlinks)   : 44 OK / 257 FAIL
#   pass 3 (shim + Win32 stubs)   : 73 OK / 228 FAIL
#   pass 4 (this run)             : ?
#
...
LOG_FILE="$LOG_DIR/first-compile-pass4.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass4.summary.txt"
...
{
    echo "# TIM-10 first compile attempt -- pass 4 (post TIM-8 + TIM-9)"
    echo "# host: $(uname -srm)"
...
# Tally include-not-found errors so we can compare against pass 3's
