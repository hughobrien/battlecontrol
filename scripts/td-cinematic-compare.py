...
EXTRA_TD_VQAS = [
    'TBRINFO1.VQA', 'GDIEND1.VQA', 'GDIEND2.VQA', 'GDIFINB.VQA',
    'BCANYON.VQA', 'BOMBFLEE.VQA', 'BURDET1.VQA', 'BURDET2.VQA',
    'CC2TEASE.VQA', 'FLAG.VQA', 'FORESTKL.VQA', 'GAMEOVER.VQA',
    'GUNBOAT.VQA', 'HELLVALY.VQA', 'LANDING.VQA', 'PINTLE.VQA',
    'PLANECRA.VQA', 'RETRO.VQA', 'SABOTAGE.VQA', 'SAMSITE.VQA',
]
...
def extract_named(mix_path: str, names: list[str], out_dir: str) -> list[tuple[str, str]]:
    ...
    for name in names:
        ...
        if data is None:
            print(f'  MISS  {name} not in {os.path.basename(mix_path)}', file=sys.stderr)
            continue
        if data[:4] != b'FORM' or data[8:12] != b'WVQA':
            print(f'  SKIP  {name} not a VQA (magic={data[:4]!r}/{data[8:12]!r})',
                  file=sys.stderr)
            continue
...
def main() -> int:
    ...
    ap.add_argument('--mix', default=MIX_DEFAULT,
    ...
    if failed:
        ...
        for r in failed:
            print(f'    [FAIL] {r["label"]}: p99={r.get("p99","?")} mean={r.get("mean","?")} '
