#!/usr/bin/env bash
# TIM-52 measurement: pass 27.
#
# Re-baseline after TIM-52 (this ticket -- 7-TU sub-cohort chain audit
# of the new dde.h:97 first-error bucket from pass-26). Captured the
# full-diagnostic stream for CONQUER.CPP (canonical TU) and confirmed
# all 7 TUs share an identical dde.h prefix: HSZ (lines 97/107/159-163),
# HDDEDATA (line 133), HCONV (line 165). Smallest-scope fix is the
# Win32 DDEML handle taxonomy in linux/win32-stubs/ddeml.h plus the
# transitive include from windows.h, mirroring TIM-51 (winsock.h) and
# TIM-46 (mmsystem.h transitive).
#
# Pre baseline (pass 26, TIM-51-confirmed): 184 OK / 117 Fail / 301 Total.
# Top of pass-26 histogram: dde.h:97 = 7 TUs, mapedit.h:170 = 2 TUs,
# WSProto.h:61 = 2 TUs, DLLInterface.h:616 = 2 TUs, then long tail of
# singletons.
#
# Same harness as passes 7-26 -- same flags, same shim regen, same
# stub set. Difference relative to pass 26 is upstream:
#   - TIM-52: linux/win32-stubs/ddeml.h is new, providing the minimum-
#             viable DDEML handle taxonomy: HSZ, HCONV, HDDEDATA,
#             HCONVLIST. linux/win32-stubs/windows.h gains
#             `#include "ddeml.h"` adjacent to the TIM-51 winsock.h
#             pull, so every TU that already force-includes
#             msvc-compat.h reaches the taxonomy. Pure typedef; no
#             shim restructuring, no flag changes.
#
...
#   pass 25 (TIM-50)                      : 182
#   pass 26 (TIM-51)                      : 184
#   pass 27 (TIM-52, this run)            : ?
#
# Measurement only -- source fix lives in TIM-52.
...
LOG_FILE="$LOG_DIR/first-compile-pass27.log"
SUMMARY_FILE="$LOG_DIR/first-compile-pass27.summary.txt"
ATTRIB_FILE="$LOG_DIR/first-compile-pass27.attribution.txt"
...
{
    echo "# TIM-52 first compile attempt -- pass 27 (post TIM-52 ddeml.h handle taxonomy + windows.h transitive)"
    echo "# host: $(uname -srm)"
