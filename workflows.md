...
```bash
# 1. Run LP64 lint (must pass — blocking gate):
#    (Include shim is auto-regenerated by CMake during build — no manual step needed)
nix run .#lint-lp64
...
| Regenerate include shim | `python3 scripts/generate-include-shim.py` (auto-run by CMake) |
